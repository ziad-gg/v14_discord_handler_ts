...
export const getUserOption = async (guild: any, userId: string) => {
    if (mongoose.connection.readyState === 0) throw new Error("Database not connected.")
    let foundUser = await UserDB.findOne({guildId: guild, userId});
    return foundUser? foundUser : new UserDB({guildId: guild, userId});
}
